...
- **5** Pillar-страница своей категории ссылается на эту статью.
  **Критерий применяется, только если `src/content/pillars/<категория>.md`
  ...
  категория оценивается по 15 баллам и нормируется к 20 (балл × 20/15),
  а не штрафуется за отсутствие того, чего в проекте нет.

### 5. Техника (20)

- **5** Бриф на визуал `src/data/visuals/<slug>.md` существует и содержит
...
- **5** Categories: ровно одна из 25 кластеров (канонический список —
