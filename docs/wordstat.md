...
`fetch.mjs` и `discover.mjs` каждый прогон создают новые датированные файлы
и папки и никогда их не удаляют сами — `discoveries/` растёт быстрее всего
...
node scripts/wordstat/cleanup-orphans.mjs [--apply] [--age 90]
```

**`prune-history.mjs`** — удаляет старые датированные дампы. Три разных
лимита, потому что разные каталоги нужны разному числу читателей:

| Каталог | Переменная | По умолчанию | Почему именно столько |
|---|---|---|---|
| `discoveries/<NS>/YYYY-MM-DD/` | `KEEP_DISCOVERIES` | 8 (минимум 2) | `diff-snapshots.mjs` сравнивает только две последние даты; запас — на случай пропущенных прогонов |
| `snapshots/YYYY-MM-DD.json` | `KEEP_SNAPSHOTS` | 8 | Не читает ни один скрипт — короткая история для ручного разбора |
| `discoveries/<NS>/diffs/*.{md,json}` | `KEEP_DIFFS` | 26 | Отчёты для человека, не вход для скриптов — маленькие, хранятся дольше |

`KEEP_DISCOVERIES` никогда не опускается ниже 2 (жёстко в коде) — иначе
`diff-snapshots.mjs` в следующий прогон не сможет построить diff вообще.
...
content-plan (статью переписали с другими ключами или удалили), но
только если запись старше `--age` дней (по умолчанию 90) — свежий ключ
не удаляется, даже если источник пока не пересобран. Предохранитель: если
источники кандидатов оказались пустыми (сбой сбора, а не «все ключи
устарели») — скрипт останавливается с ошибкой и ничего не трогает, вместо
удаления всего `keys.json` разом.

Ручной прогон при разборе: `node scripts/wordstat/prune-history.mjs --dry-run`
показывает, что было бы удалено, ничего не трогая.
...
- `POST /v2/wordstat/dynamics` — частота **точной фразы** по периодам.
  Поля: `phrase`, `period` (`PERIOD_WEEKLY` или `PERIOD_MONTHLY`),
  `fromDate`/`toDate` в **RFC3339 с временем и `Z`** (голая дата
  отклоняется), `regions: ["225"]`. Ответ — `results[].{date, count,
  share}`, `count` строкой. **1 квота на ключ.**
  Требование к `toDate` зависит от `period`: для `PERIOD_MONTHLY` — ровно
  последний день месяца, для `PERIOD_WEEKLY` — ровно воскресенье (неделя
  считается с понедельника). Нарушение — `InvalidArgument`. `fetch.mjs`
  использует `PERIOD_MONTHLY` (история для карточки ключа), а
  `enrich-candidates.mjs` — `PERIOD_WEEKLY` (свежий рост для бэклога).
...
## Динамика вместо недельного diff-а

`topRequests` не принимает дату — он всегда отдаёт срез «сейчас», поэтому
снапшот задним числом не запросить, и diff неделя-к-неделе появляется
только со второго прогона. `/dynamics` отдаёт историю на любую фразу
сразу, так что рост считается в первый же день, а не через неделю
ожидания.

`scripts/wordstat/enrich-candidates.mjs` берёт фразы из свежей выгрузки,
отсеивает навигацию и омонимы (общий фильтр — `wordstat/relevance.mjs`) и
запрашивает динамику по верхним. Результат — `candidate-dynamics.json`.

**Гранулярность — недели, не месяцы.** `PERIOD_WEEKLY`. Требование API к
границам периода то же, что для `PERIOD_MONTHLY`, только сдвинутое на
неделю: `toDate` обязан быть воскресеньем (Wordstat считает неделю с
понедельника), иначе `InvalidArgument`.

**Два разреза одной истории, а не два запроса.** Короткое окно
(`RECENT_WEEKS`, по умолчанию 3) ловит «тронулось прямо сейчас»:
разослали письмо ФНС, вышло разъяснение, наступил сезон отчётности.
Длинное (`TREND_WEEKS`, по умолчанию 16) ловит устойчивый структурный
рост, который короткое окно смазывает шумом — неделя-две спада внутри
трёхмесячного роста на короткой дистанции выглядят как ничего не
происходит. Тема с 5% роста в месяц три месяца подряд не даёт ни одного
недельного скачка и без длинного окна не попала бы в бэклог никогда — а
это как раз тема, где отставание от спроса накопилось дольше всего.

История запрашивается с запасом на длинное окно (`2×TREND_WEEKS` точек),
и оба `growth()` считаются над одними и теми же данными — это по-прежнему
1 квота на фразу. Если сработали оба сигнала разом, в бэклоге это не два
кандидата, а один и самый сильный: тема растёт и прямо сейчас, и весь
квартал.

**Ширина берётся из выгрузки, а не из динамики.** Одна выгрузка по 356
сидам даёт ~96 000 уникальных фраз, из них ~8 500 тематических от 500
показов. `/dynamics` только измеряет: одна фраза — одна квота.

Поэтому измерения **накапливаются**: каждый прогон берёт сначала
неизмеренные фразы, затем самые давние (порог `REMEASURE_DAYS=7` —
недельная динамика сдвигается каждую неделю, чаще мерить нечего), и
мержит результат с прошлым. При 700 фразах за прогон пул закрывается
примерно за дюжину запусков. Поле `coverage` в файле показывает, сколько
из пула уже измерено.

**Версия кеша.** Каждая запись несёт `cacheVersion`. При смене
гранулярности (например, с месяцев на недели) старый формат данных не
сравним с новым — «+192% за квартал» и «+80% за 3 недели» нельзя показать
рядом как одно и то же. Несовпадение версии приравнивается к отсутствию
измерения: запись перемеряется заново, а `generate-backlog.mjs`
игнорирует её, пока не перемерена. Поднимайте `CACHE_VERSION` в
`enrich-candidates.mjs` при следующей смене формата.
