...
только со второго прогона. `/dynamics` отдаёт 12 месяцев помесячной
истории на любую фразу сразу, так что рост считается в первый же день и
по более длинному окну.
...
неизмеренные фразы, затем самые давние (порог `REMEASURE_DAYS=30`), и
