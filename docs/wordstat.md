...
**Гранулярность — недели, не месяцы.** `PERIOD_WEEKLY`, окно сравнения —
последние `RECENT_WEEKS` недель (по умолчанию 3) против предыдущих
`RECENT_WEEKS`. Помесячное сравнение (три месяца против трёх предыдущих)
сглаживает ровно то, что нужно поймать: тема, которая тронулась две-три
недели назад, тонет в квартальном среднем и всплывает как «рост» только
когда уже полмесяца как растёт. Требование API к границам периода то же,
что для `PERIOD_MONTHLY`, только сдвинутое на неделю: `toDate` обязан
быть воскресеньем (Wordstat считает неделю с понедельника), иначе
`InvalidArgument`.
