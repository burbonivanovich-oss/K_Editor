...
## Общий диск — путь без обслуживания (рекомендуется)
...
Ограничение: если общий диск принадлежит организации, её администратор
может запрещать внешних участников. Сервисный аккаунт считается внешним —
тогда добавить его не получится, и остаётся путь OAuth.
...
## Семидневный срок — риск пути OAuth
