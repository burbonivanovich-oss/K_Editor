...
## Короткая инструкция (без общего диска)

Рабочий путь без Google Workspace — **OAuth от вашего личного аккаунта**:
файлы создаются от вашего имени, ложатся в ваши 15 ГБ, ключ не протухает,
пока приложение опубликовано в Production. Общий диск и сервисный аккаунт
здесь не участвуют вообще.

1. **Шаг 1** ниже — включить Sheets API, если ещё не включён.
2. **Шаг 2** — в OAuth-клиенте оставить только скоуп `drive.file`, убрать
   `webmasters.readonly` и `indexing`, если они там есть.
3. **Google Auth Platform → Audience → «Опубликовать приложение»** —
   статус должен стать `In production`. Это и снимает семидневный отзыв
   токена: подробности в «Как это работает» ниже.
4. **Шаг 3** — переполучить `refresh_token` по инструкции, положить в
   секреты `GSC_CLIENT_ID`, `GSC_CLIENT_SECRET`, `GSC_REFRESH_TOKEN`.
5. **Убрать секрет `GOOGLE_DOCS_KEY`, если он есть.** Пока он задан, код
   без явного указания пути пробует сначала его — а сервисный аккаунт без
   общего диска создать файл не может. Если убрать секрет неудобно
   (совместное использование с другими скриптами), задайте вместо этого
   `DRIVE_AUTH=oauth` — секреты можно оставить, путь будет выбран явно.
6. **`GOOGLE_DOCS_FOLDER_ID` — создать папку кнопкой, не руками.** Под
   `drive.file` приложение видит только файлы, которые создало само;
   папку, сделанную в браузере, оно не увидит и не сможет туда писать.
   Actions → «Консоль» → задача **`drive-init-root`** → вернёт `folderId`
   и ссылку. `folderId` — в секрет, ссылку — редактору.
7. Проверить: Actions → «Консоль» → **`drive-check`**. Должно быть
   `Путь аутентификации: OAuth refresh_token` и `drive.file ✅`.

Дальше — детали каждого шага, если что-то пошло не так.
...
- **Только `drive.file` и ничего больше.** Раньше здесь запрашивались ещё
  `webmasters.readonly` и `indexing` — наследие времён, когда у проекта был
  сайт и Search Console. В этом репозитории их не использует ни один скрипт,
  а вреда от них много: оба **sensitive**, и с ними Google требует
  верификации приложения для Production. Именно из-за них токен и жил
  7 дней. Убрать — значит снять семидневный срок.
- **`prompt=consent` обязателен.** Без него Google не вернёт `refresh_token`,
  если согласие уже давалось.

С одним `drive.file` (non-sensitive) экрана «Google не проверил это
приложение» быть не должно. Если он появился — в ссылке остался лишний
скоуп, проверьте её целиком.
...
## Общий диск — если есть права на него

Сервисный аккаунт не имеет собственной квоты Drive, поэтому файл, который
он создаёт в «Моём диске», принадлежит ему же и падает с
`storageQuotaExceeded`. Это не про нехватку места и не лечится уборкой.

На **общем диске** (Shared Drive) владелец файла — сам диск. Личную квоту
у создателя никто не спрашивает, и сервисный аккаунт спокойно создаёт и
таблицы, и доки. Ключ сервисного аккаунта бессрочный: обновлять раз в
неделю нечего, в отличие от OAuth.

Настройка:

1. Создать общий диск (или взять существующий) — Диск → **Общие диски**.
2. Добавить участником `client_email` из JSON сервисного аккаунта, роль
   **Менеджер контента**.
3. Создать внутри папку редакции, её ID положить в `GOOGLE_DOCS_FOLDER_ID`.
4. Проверить: `node scripts/drive-sync.mjs check` — он печатает список
   видимых общих дисков, показывает, лежит ли папка на общем диске, и
   пробует создать в ней пробный файл.

**Ограничение, которое чаще всего и срабатывает.** Если общий диск
принадлежит организации, её администратор обычно запрещает внешних
участников, а сервисный аккаунт (`…@….iam.gserviceaccount.com`) считается
внешним. Добавить его не выйдет, а завести свой общий диск без прав в
организации нельзя.

Тогда путь один — OAuth в статусе Production, см. следующий раздел. Он
даёт ровно то же: автоматическое создание файлов без обслуживания.

Код учитывает специфику общих дисков: все вызовы Drive API идут с
`supportsAllDrives`, поиск — ещё и с `includeItemsFromAllDrives`. Без этих
флагов Drive делает вид, что общих дисков не существует, и файлы в них не
находятся и не создаются.

## Как снять семидневный срок насовсем

Отзыв `refresh_token` через 7 дней — свойство статуса **Testing**, а не
OAuth вообще. Приложение в **Production** токены не отзывает.

Верификация Google при этом не нужна, если запрашивается только
`drive.file`: скоуп non-sensitive. Поэтому порядок такой:

1. Убрать из ссылки согласия всё, кроме `drive.file` (шаг 3.2).
2. **Google Auth Platform → Audience → Опубликовать приложение** →
   статус `In production`.
3. Переполучить `refresh_token` по ссылке из шага 3.2.
4. Токен больше не протухает: обновлять раз в неделю нечего.

После этого создание файлов работает автоматически и без обслуживания —
и таблиц, и доков статей.

## Корневая папка: её создаёт бот

Под `drive.file` приложение видит только те файлы, которые создало само.
Папку, сделанную человеком руками, оно не видит вообще: попытка создать
что-то внутри вернёт 404, будто папки нет. Это не ошибка прав, а смысл
скоупа.

Поэтому папку заводит бот:

```bash
node scripts/drive-sync.mjs init-root
```

Скрипт вернёт `folderId` — его в секрет `GOOGLE_DOCS_FOLDER_ID` — и ссылку
для редактора. Папка создаётся в Диске того аккаунта, под которым выдан
токен, так что владелец файлов и места — он же.
...
| `storageQuotaExceeded` | Сервисный аккаунт создаёт файл в «Моём диске», своей квоты у него нет | Перенести папку на общий диск (см. выше). Запасное — путь A |
