...
`drive-sync.mjs notify`. Живёт в **Settings → Secrets and variables →
Actions → Variables**, а не в секретах.
...
Сами адреса здесь не приводятся: строкой выше сказано, что в публичный
репозиторий их не кладём, а до 13.08.2026 прямо под этой фразой стоял
пример с двумя живыми рабочими почтами. Актуальный список — в переменной
`EDITOR_EMAILS` репозитория (Settings → Secrets and variables → Actions →
Variables) и в локальном `.env`.
...
**Адреса и почта не попадают в репозиторий.** Он публичный, и логи
Actions в нём тоже публичные. Живой список — в переменной
`EDITOR_EMAILS` и в локальном `.env`. Кнопка редактора маскирует адрес
до `д***@домен` на стороне Apps Script, чтобы полный не уезжал из Google
ни в коммит, ни в лог (`maskEmail_` в `kontur-menu.gs`).
