...
function main() {
  ...
  for (const [seed, c] of curr) {
    ...
    if (!p) {
      /* Сид добавили только что — сравнивать не с чем. Раньше такие
       * пропускались молча, и это была дыра: 11.08.2026 в seeds.json
       * добавили 36 профильных фраз, выгрузка принесла по ним 1814
       * запросов, а бэклог их не увидел вовсе — до следующей недели, когда
       * появится база для сравнения.
       *
       * Берём весь первый срез как кандидатов, но с честной пометкой: это
       * не «новая фраза в выдаче», а «сид добавлен, первая выгрузка».
       * Отбор дальше по общим правилам — полосы частотности, квоты
       * кластеров и лимит всё равно возьмут только верх списка. */
      perSeed.push({
        seed,
        slug: c.slug,
        category: c.category,
        cluster: c.cluster,
        firstRun: true,
        diff: { NEW: c.phrases.filter((x) => x.count >= MIN_COUNT), DROPPED: [], RISING: [], FALLING: [] },
      });
      continue;
    }
    perSeed.push({
