...
// Предохранитель: скрипт вызывается из GitHub Actions с --apply, и его
// коммит уходит в main автоматически. Если источники кандидатов вдруг
// оказались пустыми (файл переименовали, блог не выгрузился, сломался
// парсинг), «неактуальным» становится весь keys.json разом. Пустой
// список кандидатов — это всегда поломка, а не сигнал всё удалить.
if (activePhrases.size === 0) {
  console.error(
    "\n✗ Источники кандидатов пусты — ни одной актуальной фразы.\n" +
      "  Похоже на сбой сбора, а не на то, что ключи устарели.\n" +
      "  Уборка отменена, keys.json не тронут.",
  );
  process.exit(1);
}
