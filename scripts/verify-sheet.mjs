...
export function compare(state, pull) {
  ...
  /* Снятые темы считаем известными: строка живёт в таблице весь путь
   * темы, включая «снято» — так задумано, редактор видит, что с ней
   * решили. Пока `known` строился только по живым, первый же «убрать»
   * давал «лишняя строка» в каждом часовом прогоне: сверка кричала на
   * штатное поведение (поймано 12.08.2026 на снятом дубле). */
  const known = new Set((state.plan || []).map((t) => t.slug));
