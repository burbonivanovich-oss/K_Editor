 ...
 */
/* Длительность: число и единица. До трёх цифр — «2026 лет» не бывает, а
 * «2026 года» из даты под шаблон подпадало и превращало хвост даты в
 * отдельное значение. Одно определение на два места: ядро значения
 * статьи и нормализацию утверждения. Разъехавшись, они дают правило,
 * которое не подтверждает дословную цитату нормы. */
const DURATION_RE = /(?<![\d.,])(\d{1,3}(?:[.,]\d+)?)\s*(секунд\p{L}*|минут\p{L}*|час\p{L}*|(?:рабочих\s+|календарных\s+)?дн\p{L}*|сут\p{L}*|недел\p{L}*|месяц\p{L}*|год\p{L}*|лет)(?!\p{L})/giu;
...
export function normalizeForMatch(text) {
  ...
  /* Длительности — к тому же канону, что и ядра значений.
   *
   * Ядро значения статьи считается через `durationUnit`: «7 календарных
   * дней» → «7 дн». Норма утверждения этого преобразования не знала и
   * оставалась строкой «7 календарных дней», поэтому ядро в ней не
   * находилось никогда. Итог парадоксальный: утверждение дословно
   * повторяет срок из нормы — и именно поэтому считается его не
   * подтверждающим. Тот же класс, что «полутора секунд» и «ФФД 1.1»:
   * правило наказывало за цитату вместо пересказа. Чинится здесь, а не
   * переписыванием «календарных дней» на «дней» в статьях. */
  t = t.replace(DURATION_RE, (_m, n, unit) => `${n} ${durationUnit(unit)}`);
  t = t.replace(/№\s*/g, '№');
  t = t.replace(/\s+/g, ' ');
  return t;
}
