...
const KNOWN_DECISIONS = new Set([
  /* 23.08.2026 «одобрено» и «убрать» переименованы в «пишем» и «не
   * подходит»: колонка «Решение» сужена до двух значений — берём тему
   * или нет. Старые слова читаются по-прежнему, см. синонимы ниже. */
  'не подходит', 'пишем', 'написать сейчас', 'принято',
...
/**
 * Правка ячейки применяется к теме — кроме типа задачи и исходной ссылки.
 *
 * Тип задачи и объект актуализации — это решение, принятое на intake, а
 * не свойство текста ячейки. Пока их переписывала любая правка строки,
 * задача «Актуализировать статью https://…» тихо превращалась в тему
 * `kind: "new"`, и по ней писали новую статью вместо разбора правок к
 * существующей (13.08.2026, две темы подряд).
 *
 * Расхождение не проглатывается: оно остаётся в `kindConflict`, чтобы
 * рутина показала его человеку. Сменить тип — значит завести новую тему,
 * и это решение редактора, а не побочный эффект правки заголовка.
 */
export function applyTaskEdit(t, fields, changes = null) {
  const conflict = {};
  if (t.kind && fields.kind && fields.kind !== t.kind) conflict.declared = fields.kind;
  if (t.sourceUrl && fields.sourceUrl && fields.sourceUrl !== t.sourceUrl) conflict.sourceUrl = fields.sourceUrl;

  const { kind, sourceUrl, sourceTitle, ...rest } = fields;
  Object.assign(t, rest);
  if (!t.kind && kind) t.kind = kind;
  if (!t.sourceUrl && sourceUrl) { t.sourceUrl = sourceUrl; t.sourceTitle = sourceTitle ?? t.sourceTitle ?? null; }

  if (Object.keys(conflict).length) {
    t.kindConflict = { ...conflict, at: today(), cell: fields.rawTopic ?? null };
    if (changes?.notes) changes.notes.push(`${t.slug}: ячейка переписана на другой тип задачи — тип не меняем, нужно решение человека`);
  }
  return t;
}
...
switch (cmd) {
  ...
  case 'apply-decisions': {
    ...
    for (const row of pull.topics || []) {
      ...
      if (row.title && row.title !== (t.rawTopic ?? t.originalTitle ?? t.title)) {
        applyTaskEdit(t, taskFields(row.title), changes);
      }
      ...
      /* Формат и тезисы — заказ редактора на содержание статьи, а не
       * пометка для человека. Переносим их в состояние цикла по той же
       * причине, что и целевой запрос: то, что осталось только в
       * таблице, до автора не доходит — он работает с состоянием.
       * До 23.08.2026 формат писали прозой в «Правку», и рутина
       * разбирала его как обычное замечание, наравне с «добавь про
       * штрафы». Пустую ячейку не считаем стиранием: редактор мог
       * просто не заполнить её, а не отменить прежний заказ. */
      const format = (row.format || '').trim();
      if (format && format !== t.format) t.format = format;
      const theses = (row.theses || '').trim();
      if (theses && theses !== t.theses) t.theses = theses;
      ...
      if (d === 'не подходит' && t.status !== 'dropped') {
      ...
      } else if (d === 'пишем' && t.status === 'candidate') {
        ...
        // валидное, дропдаун только подсказывает. Опечатка вроде «не подходит!»
      ...
      if (row.note && row.note !== t.lastNote) {
        ...
        if (suspect.length) changes.suspectedInstructions.push({ slug: t.slug, title: t.title, where: 'Комментарий', what: describeFindings(suspect) });
      }
    }
   ...
   *
   * С 23.08.2026 в колонке два значения — «пишем» и «не подходит»: это
   * весь вопрос отбора темы. Остальные решения не исчезли, а появляются
   * там, где имеют смысл, и подставляет их сюда эта команда.
   *
   * Раньше в колонке было шесть значений, но одновременно применимы два-три:
  ...
  case 'row-decisions': {
    ...
    const BY_STATUS = {
      candidate: ['пишем', 'не подходит'],
      planned:  ['написать сейчас', 'не подходит'],
      writing:  ['переписать статью', 'не подходит'],
      review:   ['принято', 'переписать статью', 'не подходит'],
