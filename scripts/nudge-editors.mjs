 ...
 *
 * Адреса — в EDITOR_EMAILS (через запятую). В Actions это переменная
 * (Settings → Variables), а не секрет: рабочая почта не тайна, а видимое
 * значение проще проверить — «пришло пусто» и «пришло не то» выглядят
 * одинаково, пока значение скрыто. В репозиторий адреса всё равно не
 * кладём: он публичный. Пусто — молча выходим.
 */
