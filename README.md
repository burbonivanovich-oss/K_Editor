...
Служебных интерфейсов это не касается: пульт управления модулем —
вкладка Actions или страница с кнопками — разрешён. Страница со статьёй
для читателя нельзя, страница с кнопкой «пересобрать план» можно.
...
Пункты со стрелкой `→` консоль не выполняет: это работа агентов Claude
(`/create-article`, `/cycle-batch` и остальные). Консоль выдаёт готовую
строку для вставки в Claude Code — притворяться, что она запускает
агентов, было бы враньём.
...
| **`docs/go-live.md`** | Что осталось настроить и приёмочный прогон цикла по этапам |
