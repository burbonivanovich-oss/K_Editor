...
Чего здесь нет и не должно появиться: сайта для читателя, страниц под
контент, аналитики трафика, индексации, монетизации, лид-форм.

Служебных интерфейсов это не касается: пульт управления в Codex или
Google Drive разрешён. Страница со статьёй для читателя нельзя, а кнопка
«пересобрать план» в служебном интерфейсе допустима.

## Стек

Node.js-скрипты без внешних зависимостей — только встроенные модули.
Логика пайплайна живёт в общих markdown-процедурах и роль-брифах;
для Codex подготовлены 27 навыков и пять ролей.
Рабочее место редактора — папка Google Drive: таблица плана и по одному
Google Doc на статью.

## Запуск

В Codex открывайте непосредственно папку `repo/`, где лежит этот README,
а не родительскую `Editor/`. Начните с `AGENTS.md`.
Переезд — [docs/codex-migration.md](docs/codex-migration.md);
четыре локальные запланированные задачи и порядок включения —
[docs/routines/codex-local.md](docs/routines/codex-local.md).

```bash
npm run console      # консоль: состояние модуля + меню запуска
npm run status       # только панель состояния, без меню
```

Консоль (`scripts/console.mjs`) — одна точка входа вместо двух десятков
скриптов. Сверху панель состояния: цикл, контент, план, доступы,
свежесть Wordstat. Ниже пронумерованные пункты — проверки, цикл, статья,
Wordstat, обслуживание.

Пункты со стрелкой `→` выполняет агент. В Codex вызывайте
`$create-article`, `$cycle-batch` и другие навыки; строка `/имя` из
консоли соответствует `$имя`. В Claude Code сохраняется синтаксис `/имя`.

`node scripts/console.mjs check` проверяет, что все пункты меню указывают
на существующие скрипты. Стоит гонять после переименований.

Отдельные скрипты никуда не делись:
...
Через npm — короче: `npm run health`, `npm run cycle:check`,
`npm run cycle:state`.
...
.github/workflows/                 ручной аварийный резерв без расписания
```
...
Основной контур — шесть локальных задач Codex: M1/M2 обслуживают
Wordstat и источники, A0/A/B/C ведут темы, план, обратную связь и статьи.
Они используют `repo/.env`, Google API и локальную git-историю без
GitHub. Расписание — `docs/routines/codex-local.md`.

Старые workflow исключены из рабочего контура, их cron отключён. Меню
редактора пишет запрос напрямую в Google-таблицу.
...
| **`docs/SECRETS.md`** | Локальные ключи API и правила их обновления |
